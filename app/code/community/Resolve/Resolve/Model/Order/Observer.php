...
class Resolve_Resolve_Model_Order_Observer
{
    ...
    {
        if($this->_isResolveOrderSaveAfter) {
            Mage::dispatchEvent('resolve_action_saveorder', $observer->getData());
            $this->_isResolveOrderSaveAfter = false;
        }
    }
     ...
     *
     * @param Resolve_Resolve_Model_Payment $methodInst
     * @return bool
     */
    protected function _isCreateOrderAfterConf($methodInst)
    {
        return ($methodInst->getCode() == Resolve_Resolve_Model_Payment::METHOD_CODE) && $methodInst->redirectPreOrder();
    }
    ...
    /**
     * Call to pre order resolve action and exit
     *
...
//    }
        ...
        if (!Mage::helper('resolve')->isCheckoutFlowTypeModal()) {
            ...
            if ($this->_isCreateOrderAfterConf($methodInst)) {
                if (!Mage::helper('resolve')->getResolveTokenCode()) {
                    ...
                    Mage::helper('resolve')->getCheckoutSession()->setResolveOrderRequest(Mage::helper('core/unserializeArray')->serialize($orderRequest));
    ...
    {
        $this->_isResolveOrderSaveAfter = true;
        $quote = $observer->getQuote();
        $methodInst = $quote->getPayment()->getMethodInstance();
        if(($methodInst->getCode() == Resolve_Resolve_Model_Payment::METHOD_CODE)) {
            if (!Mage::helper('resolve')->getResolveTokenCode()) {
