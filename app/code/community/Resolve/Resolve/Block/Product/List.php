...
class Resolve_Resolve_Block_Product_List extends Mage_Catalog_Block_Product_List
{
    ...
    private function _getMinMPP()
    {
        if($this->_minMPP == null) {
            $this->_minMPP = $this->helper('resolve/promo_asLowAs')->getMinMPP();
    ...
    public function getPriceHtml($product, $displayMinimalPrice = false, $idSuffix = '')
    {
        ...
        if ($this->helper('resolve/promo_asLowAs')->isAsLowAsDisabledOnPLP()) {
            return $html;
        }
        ...
        if ($price > $mpp) {
            ...
            $mfpValue = $this->helper('resolve/promo_asLowAs')->getResolveMFPValue(array($productItemMFP), $categoryIds);

            $learnMore = ($this->helper('resolve/promo_asLowas')->isVisibleLearnMore()) ? 'true' : 'false';
            $html .= '<div class="resolve-as-low-as" data-page-type="category" ' . (!empty($mfpValue) ? 'data-promo-id="' . $mfpValue . '"' : '') . ' data-amount="' . $this->helper('resolve/util')->formatCents($price) .'" data-learnmore-show="'.$learnMore.'"></div>';
        }
