...
try {
    ...
    $setup->run("
        drop table if exists {$this->getTable('resolve/rule')};
        CREATE TABLE `{$this->getTable('resolve/rule')}` (
        ...
        drop table if exists {$this->getTable('resolve/attribute')};
        CREATE TABLE `{$this->getTable('resolve/attribute')}` (
          `attr_id` mediumint(8) unsigned NOT NULL auto_increment,
          `rule_id` mediumint(8) unsigned NOT NULL,
          `code`    varchar(255) NOT NULL default '',
          PRIMARY KEY  (`attr_id`),
          CONSTRAINT `FK_RESOLVE_RULE` FOREIGN KEY (`rule_id`) REFERENCES {$this->getTable('resolve/rule')} (`rule_id`) ON DELETE CASCADE ON UPDATE CASCADE 
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8; 
        ");
} catch (Exception $e) {
    Mage::logException($e);
