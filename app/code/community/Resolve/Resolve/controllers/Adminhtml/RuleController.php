...
class Resolve_Resolve_Adminhtml_RuleController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
	    $this->loadLayout(); 
        $this->_setActiveMenu('resolve');
        $this->_addContent($this->getLayout()->createBlock('resolve/adminhtml_' . $this->_modelName));
 	    $this->renderLayout();
    }
    ...
    public function editAction() 
    {
		...
		$model  = Mage::getModel('resolve/' . $this->_modelName)->load($id);

		if ($id && !$model->getId()) {
    		Mage::getSingleton('adminhtml/session')->addError(Mage::helper('resolve')->__('Record does not exist'));
			$this->_redirect('*/*/');
			return;
		}
		...
		Mage::register('resolve_' . $this->_modelName, $model);

		$this->loadLayout();
		
		$this->_setActiveMenu('sales/resolve/' . $this->_modelName . 's');
        ...
        $this->_addContent($this->getLayout()->createBlock('resolve/adminhtml_' . $this->_modelName . '_edit'));
        $this->_addLeft($this->getLayout()->createBlock('resolve/adminhtml_' . $this->_modelName . '_edit_tabs'));
	...
	public function saveAction() 
	{
	    ...
	    $model  = Mage::getModel('resolve/' . $this->_modelName);
		...
		if ($data) {
			...
			try {
				...
				$msg = Mage::helper('resolve')->__($this->_title . ' has been successfully saved');
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('resolve')->__('Unable to find a record to save'));
        $this->_redirect('*/*');
	} 
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($ids)) {
             Mage::getSingleton('adminhtml/session')->addError(Mage::helper('resolve')->__('Please select records'));
             $this->_redirect('*/*/');
             return;
        }
    ...
    protected function _modifyStatus($status)
    {
        ...
        if ($ids && is_array($ids)){
            try {
                Mage::getModel('resolve/' . $this->_modelName)->massChangeStatus($ids, $status);
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('system/config/resolve');
    }
}